...
class ExcelSessionManagerApp:
    ...
    def open_link_update_options(self):
        ...
        top.geometry("560x370")
        top.grab_set()
    
        frm = tk.Frame(top, padx=14, pady=12)
        frm.pack(fill="both", expand=True)
    
        tk.Label(frm, text="Only update links modified within (days):", anchor="w").grid(row=0, column=0, sticky="w")
        entry_days = tk.Entry(frm, width=6)
        ...
        update_cutoff_msg()
        tk.Label(frm, textvariable=cutoff_msg_var, fg="#0055aa", anchor="w", wraplength=440, justify="left").grid(row=1, column=0, columnspan=3, sticky="w", pady=(1, 8))
        ...
        tk.Checkbutton(frm, text="Show external link file name", variable=show_link_var).grid(row=2, column=0, columnspan=3, sticky="w")
        tk.Checkbutton(frm, text="Show external link source full path", variable=show_full_path_var).grid(row=3, column=0, columnspan=3, sticky="w")
        tk.Checkbutton(frm, text="Show last modified date", variable=show_last_modified_var).grid(row=4, column=0, columnspan=3, sticky="w")
        tk.Checkbutton(frm, text="Show update status", variable=show_status_var).grid(row=5, column=0, columnspan=3, sticky="w")
        tk.Checkbutton(frm, text="Save log file", variable=save_log_var).grid(row=6, column=0, columnspan=3, sticky="w")
    
        tk.Label(frm, text="Log file save to:", anchor="w").grid(row=7, column=0, sticky="w", pady=(6,0))
        entry_logdir = tk.Entry(frm, width=32)
        ...
        def browse_logdir():
            ...
            if d:
                ...
                self.last_log_dir = d
        browse_btn = tk.Button(frm, text="...", width=3, command=browse_logdir)
        browse_btn.grid(row=7, column=2, padx=(6,0), sticky="w", pady=(6,0))
    
        tk.Checkbutton(frm, text="Save external link scan summary", variable=save_scan_summary_var).grid(row=8, column=0, columnspan=3, sticky="w")
    
        tk.Label(frm, text="Scan summary save to:", anchor="w").grid(row=9, column=0, sticky="w", pady=(6,0))
        entry_summarydir = tk.Entry(frm, width=32)
        ...
        browse_summary_btn = tk.Button(frm, text="...", width=3, command=browse_summarydir)
        ...
        cancel_btn = tk.Button(btn_frame, text="Cancel", width=12, command=top.destroy)
        ...
        def on_confirm():
            ...
            if not os.path.isdir(log_dir):
                messagebox.showerror("Error", "The selected lock file directory does not exist. Please choose another directory.")
                return
